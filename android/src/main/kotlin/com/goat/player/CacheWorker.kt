...
class CacheWorker(
    ...
    override fun doWork(): Result {
        try {
            val data = inputData
            val url = data.getString(GoatPlayerPlugin.URL_PARAMETER)
            val cacheKey = data.getString(GoatPlayerPlugin.CACHE_KEY_PARAMETER)
            val preCacheSize = data.getLong(GoatPlayerPlugin.PRE_CACHE_SIZE_PARAMETER, 0)
            val maxCacheSize = data.getLong(GoatPlayerPlugin.MAX_CACHE_SIZE_PARAMETER, 0)
            val maxCacheFileSize = data.getLong(GoatPlayerPlugin.MAX_CACHE_FILE_SIZE_PARAMETER, 0)
            val headers: MutableMap<String, String> = HashMap()
            for (key in data.keyValueMap.keys) {
                if (key.contains(GoatPlayerPlugin.HEADER_PARAMETER)) {
                    val keySplit =
                        key.split(GoatPlayerPlugin.HEADER_PARAMETER.toRegex()).toTypedArray()[0]
