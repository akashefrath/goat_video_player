...
import com.goat.player.GoatPlayerCache.releaseCache
...
@UnstableApi
class GoatPlayerPlugin : FlutterPlugin, ActivityAware, MethodCallHandler {
    private val videoPlayers = LongSparseArray<GoatPlayer>()
    ...
    private fun stopPreCache(call: MethodCall, result: MethodChannel.Result) {
        ...
        GoatPlayer.stopPreCache(flutterState?.applicationContext, url, result)
    }

    private fun clearCache(result: MethodChannel.Result) {
        GoatPlayer.clearCache(flutterState?.applicationContext, result)
    }

    private fun getTextureId(betterPlayer: GoatPlayer): Long? {
    ...
    private fun setupNotification(betterPlayer: GoatPlayer) {
        try {
    ...
    private fun enablePictureInPicture(player: GoatPlayer) {
    ...
    private fun disablePictureInPicture(player: GoatPlayer) {
        stopPipHandler()
    ...
    private fun startPictureInPictureListenerTimer(player: GoatPlayer) {
    ...
    private fun dispose(player: GoatPlayer, textureId: Long) {
        player.dispose()
    ...
    private class FlutterState(
        ...
        fun startListening(methodCallHandler: GoatPlayerPlugin?) {
